...
<html>
    <head>
        ...
        <script>
            ...
            function mercados() {
                window.location.href = ("tianguises-usuario.html?user=" + userParam);
            }
            function catalogo() {
                window.location.href = ("productos-usuario.jsp?user=" + userParam);
            }
            function nosotros() {
                window.location.href = ("nosotros.html?user=" + userParam);
            }
            function ayuda() {
                window.location.href = ("contacto.html?user=" + userParam);
            }
            function perfil() {
            ...
            function Modificar() {
                window.location.href = ("modificar-usuario.jsp?user=" + userParam);
            }
        </script>
            ...
            <div id="container">
                ...
                <%
                    ...
                    try {
                        ...
                        conx = DriverManager.getConnection("jdbc:mysql://localhost/MercaTec?autoReconnect=true&useSSL=false", "root", "n0m3l0");
                        stat = conx.createStatement();
                    } catch (SQLException error) {
                        out.print(error.toString());
                    }
                    try {
                        result = stat.executeQuery("select*from usuarios where usuario='" + user + "'");
                        result.next();
                %>
                <form action="" method="post" id="contact_form">
                    ...
                    <div class="telephone">
                        ...
                        <br><br>
                        <input  type="text" placeholder="ADMINISTRADOR" name="admin2" value="<%=user%>" id="name_input" required>
                    </div>
                    <div class="name">
                        <label for="name">NOMBRE</label>
                        <br><br>
                        <input type="text" placeholder="Nombre" name="nom_user" pattern="^[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]+(\s*[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]*)*[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]+$"  value="<%=result.getString("nom_user")%>" id="name_input" required>
                    </div>

                    <div class="email">
                        <label for="email">APELLIDO PATERNO</label>
                        <br><br>
                        <input type="text" placeholder="Apellido Paterno" pattern="^[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]+(\s*[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]*)*[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]+$" name="ap_paterno_user"  value="<%=result.getString("ap_paterno_user")%>" required>
                    </div>

                    <div class="name">
                        <label for="name">APELLIDO MATERNO</label>
                        <br><br>
                        <input type="text" placeholder="Apellido Materno" pattern="^[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]+(\s*[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]*)*[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]+$" name="ap_materno_user" value="<%=result.getString("ap_materno_user")%>" required>
                    </div>

                    <div class="email">
                        <label for="email">EMAIL</label>
                        <br><br>
                        <input type="email" placeholder="E-mail" name="email"   id="email_input" value="<%=result.getString("email")%>" required>
                    </div>

                    <div class="email">
                        <label for="email">TELÉFONO</label>
                        <br><br>
                        <input type="text" pattern="[0-9]{10}" placeholder="Numero Telefónico" name="num_user" id="telephone_input"  value="<%=result.getString("num_user")%>" required>
                    </div>
                    <div class="name">
                        <label for="email">CONTRASEÑA</label>
                        <br><br>
                        <input type="password"  placeholder="Contraseña" name="contrasenia" id="contrasenia" value="<%=result.getString("contrasenia")%>" required>
                    </div>
                    <input type="submit" name="btngrabar" value="Cambiar" id="form_button" />
                </form><!-- // End form -->
                <%
                    } catch (SQLException error) {
                        out.print(error.toString());
                    ...
                    if (request.getParameter("btngrabar") != null) {
                        ...
                        String nom = new String(request.getParameter("nom_user").getBytes("ISO-8859-1"), "UTF-8");
                        String ap_pat = new String(request.getParameter("ap_paterno_user").getBytes("ISO-8859-1"), "UTF-8");
                        String ap_mat = new String(request.getParameter("ap_materno_user").getBytes("ISO-8859-1"), "UTF-8");
                        String email = new String(request.getParameter("email").getBytes("ISO-8859-1"), "UTF-8");
                        String num = new String(request.getParameter("num_user").getBytes("ISO-8859-1"), "UTF-8");
                        String con = new String(request.getParameter("contrasenia").getBytes("ISO-8859-1"), "UTF-8");
                        stat.execute("update usuarios set usuario='" + user2 + "', nom_user='" + nom + "', ap_paterno_user='" + ap_pat + "', ap_materno_user='" + ap_mat + "', email='" + num + "', contrasenia='" + con + "' where usuario='" + user + "';");
